...
5. 구매자는 바로 **내 서재**에서 뷰어로 읽고, 그 책에 리뷰를 쓸 수 있어요.

## 전자책 뷰어

구매(입금 확인)한 책은 사이트 안의 뷰어(`/read/책번호`)에서 읽어요. 관리자는 모든 책을 볼 수 있어요.

- **EPUB**: 장마다 본문을 정리해 사이트 글꼴로 보여 줘요. 기본 글자 크기는 모바일 17px, 태블릿 18.5px, PC 19.5px이고, 한 줄은 약 34자로 맞춰요. 독자가 글자 크기(85~170%), 명조·고딕, 줄 간격, 배경(흰색·미색·어둡게)을 고를 수 있어요(설정은 그 기기에 저장).
- **PDF**: 화면 폭에 맞춰 쪽을 그리고, 125~300%로 키울 수 있어요.
- 읽던 위치는 계정에 저장돼 다른 기기에서도 이어 읽어요.
- 파일 다운로드는 기본으로 꺼져 있고, 관리자 › 설정 › 읽기 방식에서 켤 수 있어요. PDF는 뷰어가 파일을 불러와 보여 주므로 복사를 완전히 막지는 못해요.
