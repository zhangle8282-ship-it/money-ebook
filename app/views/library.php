...
<section class="page wrap">
  ...
  <p class="muted"><?= e($user['name']) ?>님이 구매한 전자책이에요. 파일은 언제든 다시 내려받을 수 있어요.</p>
...
<?php foreach ($owned as $b): ?>
    <div class="book-card">
      ...
      <div class="book-card-info">
        ...
        <div class="book-card-author"><?= e($b['author']) ?> · <?= e($b['file_format']) ?> <?= $b['file_size'] ? e(fmt_bytes($b['file_size'])) : '' ?></div>
        <a class="btn btn-primary btn-sm library-download" href="/download/<?= (int) $b['id'] ?>">다운로드</a>
      </div>
    </div>
<?php endforeach; ?>
  </div>
<?php else: ?>
  <div class="empty">
    <p>아직 내려받을 수 있는 책이 없어요.<?= $orders ? ' 입금이 확인되면 여기에 나타나요.' : '' ?></p>
    <a href="/">전자책 둘러보기</a>
  </div>
<?php endif; ?>
